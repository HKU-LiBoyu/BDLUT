...
class DIV2K(Dataset):
    ...
    def cache_lr(self):
        lr_dict = dict()
        dataLR = os.path.join(self.path, "DIV2K_train_LR_bicubic", "X{}".format(self.scale))
    ...
    def cache_hr(self):
        hr_dict = dict()
        dataHR = os.path.join(self.path, "DIV2K_train_HR")
        for f in self.file_list:
    ...
    def __init__(self, path, scale=4):
        ...
        for dataset in ['Set5', 'Set14', 'B100', 'Urban100', 'Manga109']:
            ...
            for i in range(len(files)):
                ...
                im_lr = np.array(Image.open(os.path.join(path, dataset, 'LR_bicubic', 'X%d'%scale, files[i][:-4] + 'x%d.png'%scale)))
                ...
                key = dataset + '_' + files[i][:-4] + 'x%d' % scale # e.g. Set5_babyx4
                self.ims[key] = im_lr
