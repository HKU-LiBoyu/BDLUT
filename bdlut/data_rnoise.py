...
class DIV2K(Dataset):
    def __init__(self, scale, path, patch_size, rigid_aug=True):
        ...
        self.path = path
        self.file_list = [str(i)
                          for i in range(20088)]  # TODO:use both train and valid (901)
    ...
    def cache_lr(self):
        lr_dict = dict()
        dataLR = os.path.join(self.path, "LR_50")
    ...
    def cache_hr(self):
        hr_dict = dict()
        dataHR = os.path.join(self.path, "HR_50")
        for f in self.file_list:
...
class SRBenchmark(Dataset):
    def __init__(self, path, scale=4):
        ...
        for dataset in ['Urban100']:#['SIDD_validation']:
            ...
            for i in range(len(files)):
                ...
                im_lr = np.array(Image.open(os.path.join(path, dataset, files[i][:-4] + '.png')))
                sigma=15

                np.random.seed(seed=0)
                im_lr = im_lr + np.random.normal(0, sigma, im_lr.shape)
                ...
                key = dataset + '_' + files[i][:-4] + '_lq'# e.g. Set5_babyx4
                self.ims[key] = im_lr
