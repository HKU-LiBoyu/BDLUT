...
def SaveCheckpoint(models, opt_G, i, args, best=False):
    if best:
        if isinstance(model, nn.DataParallel):
            torch.save(model.module.state_dict(), 'checkpoint/{}/model_best.pth'.format(args.exp_name))
        else:
            torch.save(model.state_dict(), 'checkpoint/{}/model_best.pth'.format(args.exp_name))
        # torch.save(opt_G.state_dict(), 'checkpoint/{}/opt_G_best.pth'.format(args.exp_name))
        print("Best checkpoint saved")
    else:
        if isinstance(model, nn.DataParallel):
            torch.save(model.module.state_dict(), 'checkpoint/{}/model_G_i{:06d}.pth'.format(args.exp_name, i))
        else:
            torch.save(model.state_dict(), 'checkpoint/{}/model_G_i{:06d}.pth'.format(args.exp_name, i))
...
if __name__ == "__main__":
    ...
    model = Mask_demo(nf=args.n_filters, upscale=args.upscale, act=args.act_fn).cuda()
    # sr_scale = 2


    ## Optimizers
    opt_G = optim.Adam([{'params': list(filter(lambda p: p.requires_grad, model.parameters()))}], 
    ...
    # Training dataset
    train_loader = Provider(args.batch_size, args.n_workers, args.upscale, args.train_dir, args.crop_size)

    # Validation dataset
    valid_loader = SRBenchmark(args.val_dir, scale=args.upscale)
    ...
    for i in tqdm(range(args.start_iter+1, args.train_iter+1)):
        ...
        # ## Save models
        # if i % args.i_save == 0:
        #     SaveCheckpoint(model, opt_G, i, args)
            ...
            with torch.no_grad():
                ...
                for j in range(len(valid_datasets)):
                    ...
                    for k in range(len(files)):
                        ...
                        input_im = valid_loader.ims[key + 'x%d' % args.upscale] # (128, 128, 3) range [0, 255]
                        ...
                        # PSNR on Y channel
                        psnrs.append(PSNR(_rgb2ycbcr(img_gt)[:,:,0], _rgb2ycbcr(image_out)[:,:,0], args.upscale))
                    ...
                    if mean_psnr > best_psnr:
                        ...
                        SaveCheckpoint(model, opt_G, i, args, best=True)
