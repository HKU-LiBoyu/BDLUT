...
from models import *
from data_deblock import Provider, SRBenchmark
...
def parse_args():
    ...
    parser.add_argument('--msb', type=str, default='sdy', choices=['p', 'hl', 'hs', 'hd', 'sdy', 'hdv', 'hdb', 'hdbv', 'hdbl', 'hdbt'])
    parser.add_argument('--lsb', type=str, default='sdy', choices=['p', 'hl', 'hd', 'hs', 'sdy', 'hdv', 'hdb', 'hdbv', 'hdbl','hdbt'])
...
if __name__ == "__main__":
    ...
    # Training dataset
    train_loader = Provider(args.batch_size, args.n_workers, sr_scale, args.train_dir, args.crop_size, qf=args.qf)

    # Validation dataset
    valid_loader = SRBenchmark(args.val_dir, scale=sr_scale, qf = args.qf)
            ...
            with torch.no_grad():
                ...
                for j in range(len(valid_datasets)):
                    ...
                    for k in range(len(files)):
                        ...
                        input_im = valid_loader.ims[key + 'n%d' % args.qf] # (128, 128, 3) range [0, 255]
                        ...
                        # PSNR on Y channel
                        psnrs.append(bPSNR(image_out, img_gt, 0))
                    ...
                    print('Iter {} | Dataset {} | AVG Val bPSNR: {:02f}'.format(i, valid_datasets[j], mean_psnr))
                    print(valid_datasets[j],' best bpsnr is ',best_psnr_dataset[j])
                    writer.add_scalar('bPSNR_valid/{}'.format(valid_datasets[j]), mean_psnr, i)
                    writer.flush()

    print(f'Best bPSNR: {best_psnr}')
    for j in range(len(valid_datasets)):
        print('Best bPSNR for ',valid_datasets[j]," : ",best_psnr_dataset[j])
